...
function resize(){ canvas.width = Math.floor(window.innerWidth * devicePixelRatio); canvas.height = Math.floor((window.innerHeight-64) * devicePixelRatio); ctx.setTransform(devicePixelRatio,0,0,devicePixelRatio,0,0); }
window.addEventListener('resize', resize); resize();
window.addEventListener('keydown', e=>{ keys.add(e.key.toLowerCase()); if([' ','arrowup','arrowdown','arrowleft','arrowright'].includes(e.key.toLowerCase())) e.preventDefault(); if(e.key.toLowerCase()==='m') nextJob(); });
window.addEventListener('keyup', e=>keys.delete(e.key.toLowerCase()));
ui.start.onclick=()=>{ running=true; ui.intro.style.display='none'; flash('Mission started'); };
ui.closeIntro.onclick=()=>ui.intro.style.display='none';
...
function update(){
  ...
  if(player.inCar){
    ...
    if(keys.has('w')) car.speed+=0.12*boost; if(keys.has('s')) car.speed-=0.1;
    if(keys.has('a')) car.angle-=0.045*(Math.abs(car.speed)/2+0.5); if(keys.has('d')) car.angle+=0.045*(Math.abs(car.speed)/2+0.5);
    ...
    let vx=0,vy=0; if(keys.has('w'))vy--; if(keys.has('s'))vy++; if(keys.has('a'))vx--; if(keys.has('d'))vx++;
    const len=Math.hypot(vx,vy)||1; const sp=player.speed*(keys.has('shift')?1.7:1);
    player.x=clamp(player.x+vx/len*sp,20,world.w-20); player.y=clamp(player.y+vy/len*sp,20,world.h-20);
  }
